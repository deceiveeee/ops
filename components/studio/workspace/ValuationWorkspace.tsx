...
import { newValuation, saveValuation, valuationResult, type ValuationCase, type ValuationInput } from "@/lib/studio-project/valuation-cases";
...
export default function ValuationWorkspace() {
  ...
  const readCompany = async (company: Company) => {
    ...
    try {
      ...
      setNotice("Annual figures loaded. Check the figures and share ratio, then continue to assumptions.");
      await add(record);
...
function ValuationEditor({ record, alternatives, disabled }: { record: ValuationCase; alternatives: ValuationCase[]; disabled: boolean }) {
  ...
  const field = (key: ValuationInput) => <label key={key}>{fieldNames[key]}<input inputMode="decimal" value={draft.inputs[key]} maxLength={100} onChange={(e) => change({ inputs: { ...latest.current.inputs, [key]: e.target.value }, ...(key === "costOfCapital" ? { costReference: null } : {}) })} /></label>;
  ...
  return <fieldset disabled={disabled} className={styles.editor}>
    <div className={styles.tabs} aria-label="Valuation views">{(["Figures", "Assumptions", "Compare", "Sources"] as const).map((tab) => <button key={tab} aria-current={view === tab ? "page" : undefined} onClick={() => setView(tab)}>{tab}</button>)}</div>
    <div className={styles.mobileSummary}><span>{result.ok ? `Model value: ${money(result.value)} / share` : "Complete the case to estimate value"}</span><button className={styles.button} onClick={() => setView(view === "Value" ? "Assumptions" : "Value")}>{view === "Value" ? "Edit assumptions" : "See calculation"}</button></div>
    <div className={`${styles.layout} ${view === "Value" ? styles.single : ""}`}>
      {view !== "Value" && <section className={styles.panel} aria-label={`${view} for this valuation`}>
        {view === "Figures" && <><h2>{draft.company} · check the figures</h2><div className={styles.fields}>{(["nopat", "debt", "cash", "shares", "receipt"] as const).map(field)}</div><p className={styles.note}>Figures are in millions of US dollars; shares are in millions. Enter 1 for an ordinary share. For a depositary receipt, check its share ratio. Filed shares may be a yearly average.</p><button className={`${styles.button} ${styles.actions}`} onClick={() => setView("Assumptions")}>Continue to assumptions →</button></>}
        {view === "Assumptions" && <>
      ...
      <section className={`${styles.graphic} ${view !== "Value" ? styles.desktopGraphic : ""}`} aria-label="Value and market price">
        ...
        {result.ok ? <>
          ...
          {result.reverse !== null && <p>{!isValued(result.reverse) ? result.reverse.reason
            : Math.abs(Number(draft.inputs.returnOnCapital) - Number(draft.inputs.costOfCapital)) < 1e-10
...
function SourceFigures({ source }: { source: NonNullable<ValuationCase["source"]> }) {
  ...
  return <>
    ...
    <p className={styles.note}>Filed shares may be a yearly average. Check today’s outstanding share count and receipt ratio before using a value per share.</p>
  </>;
}
