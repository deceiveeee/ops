    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-white/12 bg-white/[0.03] p-5 sm:p-6">
        ...
        <p className="ops-body mt-2 text-[16px] leading-[1.6] text-slate-100">
          You estimate the stock is worth <span className="font-sans text-accent-cyan">${VALUE}</span>.
          The current market price is{" "}
          <span className="font-sans text-accent-amber">${fmt(ROUNDS[0].price)}</span>. Press{" "}
      ...
      <div className="rounded-2xl border border-white/12 bg-white/[0.03] p-5 sm:p-6">
        ...
        <div className="mt-5 space-y-3">
          ...
          <div className="flex items-center gap-3">
            <div className="w-24 flex-shrink-0 font-sans text-[11px] text-accent-cyan">Est. value</div>
            <div className="relative h-7 flex-1 overflow-hidden rounded-lg border border-white/10 bg-ink-950/40">
              ...
              <div className="absolute inset-y-0 flex items-center px-3" style={{ left: `${pctOfScale(VALUE)}%`, transform: "translateX(-100%)" }}>
                <span className="font-sans text-[12px] text-accent-cyan">${VALUE}</span>
              </div>
          ...
          {/* Current price */}
          <div className="flex items-center gap-3">
            <div className="w-24 flex-shrink-0 font-sans text-[11px] text-accent-amber">Price</div>
            <div className="relative h-7 flex-1 overflow-hidden rounded-lg border border-white/10 bg-ink-950/40">
              <motion.div
              ...
              <motion.div
                ...
                transition={reduce ? { duration: 0 } : { duration: 0.4, ease: "easeOut" }}
              >
                <span className="whitespace-nowrap font-sans text-[12px] text-accent-amber">${fmt(current.price)}</span>
              </motion.div>
            </div>
          </div>
          ...
          {/* Potential gain */}
          <div className="flex items-center gap-3">
            <div className="w-24 flex-shrink-0 font-sans text-[11px] text-accent-green">Gain left</div>
            <div className="relative h-7 flex-1 overflow-hidden rounded-lg border border-white/10 bg-ink-950/40">
              <motion.div
        ...
        <div className="mt-4 flex items-center gap-2">
          <span className="font-sans text-[11px] uppercase tracking-[0.14em] text-slate-400">Competing investors:</span>
