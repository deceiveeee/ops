          ...
          <svg viewBox={`0 0 ${W} ${H}`} className="w-full min-w-[560px]" role="img" aria-label="Spot rate timeline and forward rate timeline">
            ...
            {Array.from({ length: MAX_T + 1 }).map((_, i) => (
              <g key={i}>
                ...
                <text x={xAt(i)} y={215} textAnchor="middle" className="fill-slate-400 font-mono" fontSize="12">
                  {i === 0 ? "t=0" : `${i}yr`}
                </text>
              </g>
            ))}
            ...
            <text x={padX - 6} y={spotY} textAnchor="end" className="fill-accent-cyan font-mono" fontSize="12">
              Spot
            </text>
            ...
            <text x={(xAt(0) + xAt(MAX_T)) / 2} y={spotY - 14} textAnchor="middle" className="fill-slate-300 font-mono" fontSize="12">
              money moves today → future (solid)
            </text>

            {/* FORWARD ROW — agree today dotted, transaction t1→t2 dashed */}
            <text x={padX - 6} y={fwdY} textAnchor="end" className="fill-accent-purple font-mono" fontSize="12">
              Forward
            </text>
            ...
            <text x={xAt(0)} y={fwdY - 16} textAnchor="middle" className="fill-accent-purple font-mono" fontSize="11">
              agree today
            </text>
            ...
            <text x={(xAt(2) + xAt(4)) / 2} y={fwdY - 14} textAnchor="middle" className="fill-slate-300 font-mono" fontSize="12">
        ...
        <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-2">
          <div className="rounded-2xl border border-accent-cyan/25 bg-accent-cyan/[0.05] p-5">
            ...
            <p className="ops-body mt-2 text-[15px] leading-7 text-slate-200">
              ...
              that moves <span className="font-mono">now</span>. You hand over
          ...
          <div className="rounded-2xl border border-accent-purple/25 bg-accent-purple/[0.05] p-5">
            ...
            <p className="ops-body mt-2 text-[15px] leading-7 text-slate-200">
              ...
              <span className="font-mono">later</span>. The rate is fixed today,
