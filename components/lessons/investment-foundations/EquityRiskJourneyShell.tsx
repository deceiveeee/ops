...
export type EquityRiskJourneyStep = {
  label: string;
  title: string;
  guide: string;
  instruction: string;
...
export type EquityRiskSceneProps = { onComplete: () => void };

export default function EquityRiskJourneyShell({
  lessonSlug,
  ariaLabel,
  steps,
  renderStep,
  ...
  lessonSlug: string;
  ariaLabel: string;
  steps: readonly EquityRiskJourneyStep[];
  ...
  return (
    ...
    >
      <div className="ops-interactive-frame overflow-hidden p-0">
        ...
        <div className="border-b border-white/10 bg-[radial-gradient(circle_at_5%_0%,rgba(34,211,238,0.09),transparent_36%),radial-gradient(circle_at_22%_0%,rgba(251,191,36,0.08),transparent_42%)] px-5 py-5 sm:px-7">
        ...
        <div className="border-t border-white/10 bg-white/[0.025] px-5 py-4 sm:px-7">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="order-1 text-center sm:order-2 sm:max-w-xs">
              ...
              >
                {currentComplete
                  ? "Mission complete. The next activity uses this evidence."
                  : current.instruction}
              </div>
            </div>
...
export function RiskPrompt({ children }: { children: ReactNode }) {
  return (
...
export function RiskFeedback({
  ...
  return (
    ...
    >
      ...
      >
        {correct ? "Connected." : "Recheck the evidence."}
      </span>
      {children}
    </div>
  );
...
export function RiskMetric({
  ...
  return (
    <div className={cn("rounded-xl border bg-white/[0.025] p-4", tones[tone])}>
