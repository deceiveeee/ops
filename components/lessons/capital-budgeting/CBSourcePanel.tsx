...
import { CB_SOURCES } from "./shared";

export default function CBSourcePanel({ sources = CB_SOURCES }: { sources?: string[] }) {
  return (
    <aside
      ...
      <ul className="mt-4 space-y-2.5">
        {sources.map((s) => (
          <li key={s} className="ops-muted flex items-start gap-2.5 text-[13px]">
            <span className="mt-1.5 h-1.5 w-1.5 flex-shrink-0 rounded-full bg-accent-amber/60" aria-hidden />
            {s}
          </li>
        ))}
      </ul>
      <p className="ops-muted mt-4 text-[12px]">
        Lesson content is adapted from MIT 15.401 capital-budgeting material for educational use.
        All cash flows are described as expected, not guaranteed. Examples use illustrative figures,
        not current market estimates.
      </p>
    </aside>
  );
}
