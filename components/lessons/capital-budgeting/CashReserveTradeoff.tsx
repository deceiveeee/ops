    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div className="rounded-xl border border-accent-amber/25 bg-accent-amber/[0.05] p-4">
          <div className="font-mono text-[10px] uppercase tracking-[0.16em] text-accent-amber">Estimated minimum reserve</div>
          <div className="mt-1.5 font-mono text-[20px] text-white">${fmt(minReserve)}M</div>
        </div>
        <div className="rounded-xl border border-white/10 bg-ink-950/40 p-4">
          <div className="font-mono text-[10px] uppercase tracking-[0.16em] text-slate-400">Current balance</div>
          <div className="mt-1.5 font-mono text-[20px] text-white">${fmt(cashBalance)}M</div>
        </div>
        <div className={cn("rounded-xl border p-4", excess > 0 ? "border-accent-green/25 bg-accent-green/[0.05]" : "border-accent-red/25 bg-accent-red/[0.05]")}>
          <div className={cn("font-mono text-[10px] uppercase tracking-[0.16em]", excess > 0 ? "text-accent-green" : "text-accent-red")}>{excess > 0 ? "Excess cash" : "Shortfall"}</div>
          <div className={cn("mt-1.5 font-mono text-[20px]", excess > 0 ? "text-accent-green" : "text-accent-red")}>${fmt(Math.abs(excess))}M</div>
        </div>
      </div>

      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <div className="rounded-2xl border border-accent-green/25 bg-accent-green/[0.04] p-5">
          <div className="font-mono text-[11px] uppercase tracking-[0.16em] text-accent-green">Benefits of holding cash</div>
        ...
        <div className="rounded-2xl border border-accent-red/25 bg-accent-red/[0.04] p-5">
          <div className="font-mono text-[11px] uppercase tracking-[0.16em] text-accent-red">Costs of holding cash</div>
          <ul className="mt-2 space-y-1.5">
